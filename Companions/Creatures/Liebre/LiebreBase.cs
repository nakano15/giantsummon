    ...
    public class LiebreBase : GuardianBase
    {
        ...
        public override void GuardianPostDrawScript(TerraGuardian guardian, Vector2 DrawPosition, Color color, Color armorColor, float Rotation, Vector2 Origin, float Scale, SpriteEffects seffect)
        {
            ...
            byte MouthFrameX = 0;
            {
                ...
                const int SparkleDelay = 7500;
                while (SoulsValue > 0)
                {
                    if (Main.rand.NextFloat() < (SoulsValue / (SparkleDelay + SoulsValue)) * OpacityRate)
                    {
        ...
        {
            List<string> Mes = new List<string>();
            Mes.Add("*There's actually one thing you can help me with, if you are interessed... I need you to [objective].*");
            Mes.Add("*I've been so busy with my job, that I forgot to take care of a thing... Gladly, you can do It instead, if you agree to. It's to [objective], what do you say?*");
            return Mes[Main.rand.Next(Mes.Count)];
        }
        ...
        public override string GetSpecialMessage(string MessageID)
        {
            switch (MessageID)
            {
                ...
                case MessageIDs.DeliveryItemMissing:
                    return "*Hm.. The item I was going to give is gone.*";
                case MessageIDs.DeliveryInventoryFull:
                    return "*I wanted to give [target] something, but their inventory is full.*";
                case MessageIDs.CommandingLeadGroupSuccess:
                    return "*I shall do so for you. Lead the souls to me.*";
                case MessageIDs.CommandingLeadGroupFail:
                    return "*My job here is to deliver souls to their right places, so I can't do that.*";
                case MessageIDs.CommandingDisbandGroup:
                    return "*Everyone should get their rest, then.*";
                case MessageIDs.CommandingChangeOrder:
                    return "*That is how I will lead the souls now.*";
            }
