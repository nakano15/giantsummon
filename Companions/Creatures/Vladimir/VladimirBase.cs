    ...
    public class VladimirBase : GuardianBase
    {
        ...
        public void HugOptionAction()
        {
            ...
            if (Guardian.DoAction.InUse && Guardian.DoAction.ID == HugActionID && Guardian.DoAction.IsGuardianSpecificAction)
            {
                ...
                Main.npcChatText = ((Companions.VladimirBase)Guardian.Base).GetEndHugMessage(Guardian);
                ((Companions.Vladimir.HugAction)Guardian.DoAction).Target.Bottom = Guardian.Position;
            }
            else if (!Guardian.DoAction.InUse)
            {
                ...
                {
                    if (Guardian.StartNewGuardianAction(new Companions.Vladimir.HugAction(Main.player[Main.myPlayer]), HugActionID))
                    {
        ...
        public override string GetSpecialMessage(string MessageID)
        {
            switch (MessageID)
            {
                ...
                case MessageIDs.FeatPlayerMetMe:
                    return "*I have got a new friend recently. Their name was [player].*";
                case MessageIDs.FeatCompletedAnglerQuests:
                    return "*I'm really glad that [player] got really into fishing. That makes me remember of when we met.*";
                ...
                case MessageIDs.FeatMentionSomeoneMovingIntoAWorld:
                    return "*[subject] now has a new house in [world].*";
                ...
                case MessageIDs.CommandingLeadGroupSuccess:
                    return "*I shall care for my group, like you care for me.*";
                case MessageIDs.CommandingLeadGroupFail:
                    return "*I'm sorry to disappoint, but I can't do that.*";
                case MessageIDs.CommandingDisbandGroup:
                    return "*I hope my leadership was satisfactory.*";
                case MessageIDs.CommandingChangeOrder:
                    return "*I will change my leadership behavior then.*";
            }
